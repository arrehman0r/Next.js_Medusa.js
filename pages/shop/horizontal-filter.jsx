                    ...
                    <div className="select-items">
                        {
                            filterData.sizes.map( ( item, index ) =>
                                containsAttrInUrl( 'sizes', item.id ) ? <ALink className="select-item" href={ { pathname: router.pathname, query: { ...query, page: 1, sizes: getUrlForAttrs( 'sizes', item.id ), type: router.query.type ? router.query.type : null } } } key={ item + ' - ' + index } onClick={ selectFilterHandler } scroll={ false }>{ item.title }<i className="d-icon-times"></i></ALink> : ''
                            )
                        }
                        {
                            filterData.colors.map( ( item, index ) =>
                                containsAttrInUrl( 'colors', item.id ) ? <ALink className="select-item" href={ { pathname: router.pathname, query: { ...query, page: 1, colors: getUrlForAttrs( 'colors', item.id ), type: router.query.type ? router.query.type : null } } } key={ item + ' - ' + index } onClick={ selectFilterHandler } scroll={ false }>{ item.title }<i className="d-icon-times"></i></ALink> : ''
                            )
                        }
                        {
